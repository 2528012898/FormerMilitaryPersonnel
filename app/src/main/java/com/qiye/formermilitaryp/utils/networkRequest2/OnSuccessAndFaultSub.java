...
//            final String result = CompressUtils.decompress(body.byteStream());
            ...
            String error = result.substring(0, 4);
            if ("<pre".equals(error)) {
                mOnSuccessAndFaultListener.onFault(error);
            }
            JSONObject jsonObject = new JSONObject(result);
            if (jsonObject.isNull("retCode")) {
                //resultCode = jsonObject.getString("status");
                mOnSuccessAndFaultListener.onFault("retCode not exist");
                return;
            } else {
                resultCode = jsonObject.getString("retCode");
            }
            if (resultCode.equals("0")) {//成功
                mOnSuccessAndFaultListener.onSuccess(result);
            } else {//2 可能为未登录情况暂且不考虑
                String errorMsg = jsonObject.getString("message");
                if (TextUtils.isEmpty(errorMsg))errorMsg="message is null";
